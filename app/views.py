...
def facturas(request, mesa):
    ...
    total = Menu.objects.raw("SELECT app_mesas.id, SUM(app_menu.valor*app_destallepedido.cantidad) as total FROM app_destallepedido RIGHT JOIN app_pedido ON app_pedido.id = app_destallepedido.pedido_id INNER JOIN app_menu ON app_menu.id = app_destallepedido.articulo_id INNER JOIN app_mesas ON app_pedido.mesa_id=app_mesas.id  WHERE app_pedido.restaurante_id='2250076' AND app_pedido.mesa_id='1' AND app_pedido.estado='AC' GROUP BY app_mesas.id")
